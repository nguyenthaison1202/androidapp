...
public class ElectronicList extends AppCompatActivity {
    RecyclerView rcv_electronic;
    AdapterElectronic adapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        rcv_electronic = findViewById(R.id.rcv_electronic);
        adapter = new AdapterElectronic(this);
        LinearLayoutManager linearLayoutManager = new LinearLayoutManager(this);
        rcv_electronic.setLayoutManager(linearLayoutManager);
        RecyclerView.ItemDecoration itemDecoration = new DividerItemDecoration(this,DividerItemDecoration.VERTICAL);
        rcv_electronic.addItemDecoration(itemDecoration);
        items = new ArrayList<>();
        adapter.setData(items);
        rcv_electronic.setAdapter(adapter);
        getItems();
    }
        ...
        databaseReference.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                for(DataSnapshot dataSnapshot : snapshot.getChildren())
                {
                    ...
                    if(verify.equals("1") && danhmuc.equalsIgnoreCase("Đồ điện tử")){
